...
module Yus
  module Persistence
    class Odba
      def initialize
        @entities = ODBA.cache.fetch_named("entities", self) { {} }
      end
  ...
  class Entity
    ...
    alias_method :odba_join, :join
    alias_method :odba_leave, :leave
    def join(entity)
  ...
  class Server
    alias_method :odba_login_entity, :login_entity
    alias_method :odba_login_root, :login_root
